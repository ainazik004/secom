...
// 🔧 TEMP SWITCH:
//  - set false to verify callable/auth pipeline works
//  - set true after App Check is stable on your device/emulator
...
exports.spendZhalbyraks = onCall(
  ...
  async (req) => {
    ...
    // ✅ App Check (make the error explicit when enforcement is ON)
    // When ENFORCE_APP_CHECK is true, Firebase will normally block before your code runs,
    // but this keeps messages consistent and debuggable.
    if (ENFORCE_APP_CHECK && !req.app) {
    ...
    const ref = asString(body.ref).trim();
